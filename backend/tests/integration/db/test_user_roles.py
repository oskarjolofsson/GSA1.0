from core.infrastructure.db.models.UserRole import UserRole
from core.infrastructure.db.models.Role import Role
from core.infrastructure.db.repositories.user_roles import (
    get_user_role,
    get_roles_by_user_id,
    get_user_roles_by_user_id,
    get_users_by_role_id,
    user_has_role,
    assign_role_to_user,
    remove_role_from_user,
    remove_all_roles_from_user,
)
from core.infrastructure.db.repositories.roles import create_role
import pytest
import uuid

# ----------- FIXTURES ------------

@pytest.fixture
def test_role(db_session):
    """Create a role for testing"""
    role = Role(name=f"test_role_{uuid.uuid4().hex[:8]}")
    return create_role(role=role, session=db_session)


@pytest.fixture
def test_role_admin(db_session):
    """Create an admin role for testing"""
    role = Role(name=f"admin_{uuid.uuid4().hex[:8]}")
    return create_role(role=role, session=db_session)


# ----------- CREATE ------------

def test_assign_role_to_user(db_session, test_user, test_role):
    """Test assigning a role to a user"""
    user_role = UserRole(
        user_id=test_user["user_id"],
        role_id=test_role.id,
    )

    created = assign_role_to_user(user_role=user_role, session=db_session)

    assert created.user_id == test_user["user_id"]
    assert created.role_id == test_role.id
    

def test_assign_multiple_roles_to_user(db_session, test_user, test_role, test_role_admin):
    """Test assigning multiple roles to a user"""
    roles_before = get_roles_by_user_id(test_user["user_id"], session=db_session)
    
    user_role1 = UserRole(user_id=test_user["user_id"], role_id=test_role.id)
    user_role2 = UserRole(user_id=test_user["user_id"], role_id=test_role_admin.id)

    assign_role_to_user(user_role=user_role1, session=db_session)
    assign_role_to_user(user_role=user_role2, session=db_session)

    roles = get_roles_by_user_id(test_user["user_id"], session=db_session)

    print([r.name for r in roles])  # Debug print to verify roles
    assert len(roles) - len(roles_before) == 2


# ------------ READ ------------

def test_get_user_role(db_session, test_user, test_role):
    """Test retrieving a specific user-role assignment"""
    user_role = UserRole(user_id=test_user["user_id"], role_id=test_role.id)
    assign_role_to_user(user_role=user_role, session=db_session)

    fetched = get_user_role(test_user["user_id"], test_role.id, session=db_session)

    assert fetched is not None
    assert fetched.user_id == test_user["user_id"]
    assert fetched.role_id == test_role.id

def test_get_user_role_not_found(db_session, test_user, test_role):
    """Test retrieving non-existent user-role returns None"""
    result = get_user_role(test_user["user_id"], test_role.id, session=db_session)

    assert result is None

def test_get_roles_by_user_id(db_session, test_user, test_role, test_role_admin):
    """Test retrieving all roles for a user"""
    roles_before = get_roles_by_user_id(test_user["user_id"], session=db_session)
    
    user_role1 = UserRole(user_id=test_user["user_id"], role_id=test_role.id)
    user_role2 = UserRole(user_id=test_user["user_id"], role_id=test_role_admin.id)
    assign_role_to_user(user_role=user_role1, session=db_session)
    assign_role_to_user(user_role=user_role2, session=db_session)

    roles = get_roles_by_user_id(test_user["user_id"], session=db_session)

    assert len(roles) - len(roles_before) == 2
    role_ids = [r.id for r in roles]
    assert test_role.id in role_ids
    assert test_role_admin.id in role_ids

def test_get_user_roles_by_user_id(db_session, test_user, test_role):
    """Test retrieving UserRole records for a user"""
    roles_before = get_user_roles_by_user_id(test_user["user_id"], session=db_session)
    
    user_role = UserRole(user_id=test_user["user_id"], role_id=test_role.id)
    assign_role_to_user(user_role=user_role, session=db_session)

    user_roles = get_user_roles_by_user_id(test_user["user_id"], session=db_session)

    assert len(user_roles) - len(roles_before) == 1
    assert user_roles[-1].user_id == test_user["user_id"]
    assert user_roles[-1].role_id == test_role.id

def test_get_users_by_role_id(db_session, test_user, test_role):
    """Test retrieving all users with a specific role"""
    user_role = UserRole(user_id=test_user["user_id"], role_id=test_role.id)
    assign_role_to_user(user_role=user_role, session=db_session)

    users: list[uuid.UUID] = get_users_by_role_id(test_role.id, session=db_session)

    assert test_user["user_id"] in users

def test_user_has_role_true(db_session, test_user, test_role):
    """Test checking if user has a specific role - positive case"""
    user_role = UserRole(user_id=test_user["user_id"], role_id=test_role.id)
    assign_role_to_user(user_role=user_role, session=db_session)

    result = user_has_role(test_user["user_id"], test_role.name, session=db_session)

    assert result is True

def test_user_has_role_false(db_session, test_user, test_role):
    """Test checking if user has a specific role - negative case"""
    result = user_has_role(test_user["user_id"], test_role.name, session=db_session)

    assert result is False


# ------------ DELETE ------------

def test_remove_role_from_user(db_session, test_user, test_role):
    """Test removing a role from a user"""
    user_role = UserRole(user_id=test_user["user_id"], role_id=test_role.id)
    assign_role_to_user(user_role=user_role, session=db_session)

    remove_role_from_user(test_user["user_id"], test_role.id, session=db_session)

    fetched = get_user_role(test_user["user_id"], test_role.id, session=db_session)
    assert fetched is None

def test_remove_nonexistent_role_from_user(db_session, test_user, test_role):
    """Test removing a role that user doesn't have (should not error)"""
    # Should not raise an exception
    remove_role_from_user(test_user["user_id"], test_role.id, session=db_session)

def test_remove_all_roles_from_user(db_session, test_user, test_role, test_role_admin):
    """Test removing all roles from a user"""
    roles_before = get_roles_by_user_id(test_user["user_id"], session=db_session)
    
    user_role1 = UserRole(user_id=test_user["user_id"], role_id=test_role.id)
    user_role2 = UserRole(user_id=test_user["user_id"], role_id=test_role_admin.id)
    assign_role_to_user(user_role=user_role1, session=db_session)
    assign_role_to_user(user_role=user_role2, session=db_session)

    remove_all_roles_from_user(test_user["user_id"], session=db_session)

    roles = get_roles_by_user_id(test_user["user_id"], session=db_session)
    assert len(roles) == 0


## ------------ CONSTRAINTS & EDGE CASES ------------

def test_user_role_composite_primary_key(db_session, test_user, test_role):
    """Test that user-role combination is unique"""
    user_role1 = UserRole(user_id=test_user["user_id"], role_id=test_role.id)
    assign_role_to_user(user_role=user_role1, session=db_session)

    user_role2 = UserRole(user_id=test_user["user_id"], role_id=test_role.id)
    with pytest.raises(Exception):
        assign_role_to_user(user_role=user_role2, session=db_session)

