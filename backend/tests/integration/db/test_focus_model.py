...
def test_program_graduates_when_all_drills_grooved(db_session, test_user):
    ...
    program = ps.generate_program(user_id, db_session, analysis_issue_id=analysis_issue.id)
...
def test_two_active_programs_in_one_area_are_allowed(db_session, test_user):
    ...
    p_a = ps.generate_program(user_id, db_session, analysis_issue_id=ai_a.id)
    p_b = ps.generate_program(user_id, db_session, analysis_issue_id=ai_b.id)
...
def test_third_active_program_in_one_area_is_blocked(db_session, test_user):
    ...
    ps.generate_program(user_id, db_session, analysis_issue_id=ai_a.id)
    ps.generate_program(user_id, db_session, analysis_issue_id=ai_b.id)
    with pytest.raises(exceptions.ConflictException):
        ps.generate_program(user_id, db_session, analysis_issue_id=ai_c.id)
...
def test_areas_are_capped_independently(db_session, test_user):
    ...
    ps.generate_program(user_id, db_session, analysis_issue_id=ai_a.id)
    ps.generate_program(user_id, db_session, analysis_issue_id=ai_b.id)
    putting = ps.generate_program(user_id, db_session, analysis_issue_id=ai_p.id)
...
def test_completing_a_program_frees_its_slot(db_session, test_user):
    ...
    p_c = ps.generate_program(user_id, db_session, analysis_issue_id=ai_c.id)
    assert p_c.slot == p_a.slot  # reuses the freed slot rather than inventing a third


def test_second_program_on_the_same_issue_returns_the_first(db_session, test_user):
    """Two programs on one issue would groove identical drill sets against separate
    counters with nothing to make them diverge, so the unique index forbids it.

    Asking again hands back the program you already have. Both entry points agree on that
    now -- the browse path used to raise 409 here while the AI path returned the existing
    program, so the same action succeeded or failed depending on which screen it came
    from."""
    user_id = test_user["user_id"]
    _, ai = _seed_issue(db_session, user_id, "Issue A", 0.9, num_drills=2)

    first = ps.generate_program(user_id, db_session, analysis_issue_id=ai.id)
    second = ps.generate_program(user_id, db_session, analysis_issue_id=ai.id)
...
def test_generate_still_idempotent_for_same_issue(db_session, test_user):
    user_id = test_user["user_id"]
    _, ai = _seed_issue(db_session, user_id, "Issue A", 0.9, num_drills=2)
    p1 = ps.generate_program(user_id, db_session, analysis_issue_id=ai.id)
    p2 = ps.generate_program(user_id, db_session, analysis_issue_id=ai.id)  # not blocked
    assert p1.id == p2.id
...
def test_slot_race_reports_a_retry_not_a_false_full_house(db_session, test_user, monkeypatch):
    ...
    ps.generate_program(user_id, db_session, analysis_issue_id=ai_a.id)  # takes slot 0
...
def test_get_next_step_does_not_write(db_session, test_user):
    ...
    user_id = test_user["user_id"]
    _, ai = _seed_issue(db_session, user_id, "Issue A", 0.9, num_drills=2)
    program = ps.generate_program(user_id, db_session, analysis_issue_id=ai.id)
...
def test_todays_issue_is_active_program_issue(db_session, test_user):
    ...
    ps.generate_program(user_id, db_session, analysis_issue_id=ai_focus.id)
...
def test_todays_issue_picks_among_several_active_programs(db_session, test_user):
    ...
    ps.generate_program(user_id, db_session, analysis_issue_id=ai_low.id)
    ps.generate_program(user_id, db_session, analysis_issue_id=ai_high.id)
