...
def modify_analysis_issue(analysis_issue: models.AnalysisIssue, session: Session) -> models.AnalysisIssue:
...
def get_analysis_issues_by_user_id_and_issue_id(user_id, issue_id, session: Session) -> models.AnalysisIssue | None:
    return (
...
def get_analysis_issues_by_issue_ids(issue_ids: list[UUID], session: Session) -> list[models.AnalysisIssue]:
    return session.query(models.AnalysisIssue).filter(models.AnalysisIssue.issue_id.in_(issue_ids), models.AnalysisIssue.active == True).all()
