...
/**
 * The photographic header: picture, scrim, greeting, and the screen's one chrome control
 * (`+` opens the focus drawer).
 *
 * The scrim is a gradient, which DESIGN.md otherwise forbids, and the control is cream
 ...
 */
export default function HomeHero({ image, title, subtitle, onAddFocus }: Props) {
  ...
  return (
    <View style={{ height: HERO_HEIGHT }} className="w-full overflow-hidden bg-ink">
      ...
      <View className="flex-row items-start px-6" style={{ paddingTop: insets.top + 12 }}>
        <Pressable
          onPress={onAddFocus}
          hitSlop={8}
          ...
          <Plus size={20} color={colors.sand} strokeWidth={2} />
        </Pressable>
      </View>

      {/* The greeting drops below the control rather than sharing its row. */}
