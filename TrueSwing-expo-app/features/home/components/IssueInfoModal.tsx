...
type IssueInfoModalProps = {
  ...
  onClose: () => void;
  onRemove?: () => void;
  /** Opens this issue's swing history. Optional: the sheet is also used where
  ...
  return (
    <Modal
      ...
      <View className="flex-1 justify-end bg-black/60">
        <SafeAreaView
          edges={['bottom']}
          className="max-h-[80%] rounded-t-3xl border border-b-0 border-white/10 bg-ink-raised">
          <ScrollView contentContainerStyle={{ padding: 24 }}>
            ...
            {/* Swing history used to hang off the home card, which no
                            longer exists. This sheet is the per-issue detail view
                            now, so it carries both of that card's actions. */}
            {onOpenHistory && (
              <Pressable
                ...
                style={{ minHeight: 44 }}>
                <Text className="font-sans-semibold text-[14px] text-sand">
                  See your swings for this issue
                </Text>
              </Pressable>
            )}
            ...
            <Pressable
              onPress={onClose}
              className="mt-4 items-center rounded-2xl border border-white/10 bg-white/5 py-4 active:bg-white/10">
