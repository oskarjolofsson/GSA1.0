...
describe('promptFor', () => {
  it('prefers the authored label', () => {
    expect(promptFor({ type: 'make_rate', label: '6-foot putts made' })).toBe('6-foot putts made');
  });
  ...
  it('falls back to an open question for an unknown type', () => {
    // The wording moved off "feel" -- the picker is now labelled Poor / OK / Very good,
    // so asking "how did that block feel" set up a scale the buttons did not offer.
    expect(promptFor({ type: 'invented_later' })).toMatch(/how did that block go/i);
    expect(promptFor(null)).toMatch(/how did that block go/i);
  });

  it('punctuates its fallbacks as questions, because BlockRating renders them as one', () => {
    expect(promptFor({ type: 'make_rate' })).toMatch(/\?$/);
    expect(promptFor({ type: 'proximity' })).toMatch(/\?$/);
    expect(promptFor(null)).toMatch(/\?$/);
  });

  it('passes an authored label through untouched, question or not', () => {
    expect(promptFor({ type: 'make_rate', label: 'Fairways hit out of 14' })).toBe(
...
describe('gradePreview — mirrors backend/core/services/drill_metrics.py', () => {
  ...
  it('captions only real grades', () => {
    expect(gradeCaption('dialed')).toMatch(/very good/i);
    expect(gradeCaption(null)).toBeNull();
    expect(gradeCaption('invented_later')).toBeNull();
  });

  /**
   * The caption names what the block was WORTH, not how it felt.
   *
   * `GRADE_STRENGTH_DELTA` is `{rough: -1, ok: 0, dialed: +1}` and a drill fills in at
   * strength 3 (`backend/core/services/program_service.py:31-36`). So an "ok" block moves
   * the golfer exactly nowhere, and this caption used to call it "Solid for this drill" --
   * ten OK sessions, a fraction that never budges, and no way to find out why.
   */
  it('does not tell the golfer an OK block earned progress', () => {
    expect(gradeCaption('ok')).toMatch(/no change/i);
    expect(gradeCaption('ok')).not.toMatch(/solid/i);
    expect(gradeCaption('rough')).toMatch(/sets this drill back/i);
    expect(gradeCaption('dialed')).toMatch(/counts toward/i);
  });
});
