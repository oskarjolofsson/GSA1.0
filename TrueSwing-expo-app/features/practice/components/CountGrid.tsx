 ...
 *
 * Scrolls rather than shrinks past reps: 10. A 20-rep drill gets a taller grid the golfer
 * scrolls; shrinking the tiles to fit would quietly reintroduce the touch-target problem
    ...
    return (
        <Pressable
            ...
            className={`h-14 flex-1 items-center justify-center rounded-2xl border ${
                selected
                    ? 'border-gold bg-gold'
                    : 'border-white/10 bg-ink-raised active:bg-white/10'
            } ${disabled ? 'opacity-40' : ''}`}
        >
            <Text
                className={`text-2xl font-display-bold ${selected ? 'text-ink' : 'text-sand'}`}
            >
                {n}
            </Text>
        </Pressable>
