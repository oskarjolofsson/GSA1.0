...
All calls to the backend go through `apiClient` (`get/post/patch/put/delete`). It pulls the current Supabase access token, attaches `Authorization: Bearer …`, and throws a typed `ApiError` (`lib/errors.ts`) with status + code on failure. Base URL comes from `EXPO_PUBLIC_API_URL`. Feature `services/` files (e.g. `features/analysis/services/analysisService.ts`) wrap this client — do not call `fetch` directly from hooks/components. API paths live in `lib/api/routes.ts` (no `/api/v1/...` string literals in feature code). See `lib/README.md` for the full boundary.

**API types are generated, not hand-written.** Feature request/response types alias out of `lib/api/schema.d.ts`, produced by `openapi-typescript` from the backend's `/openapi.json`. This is **manual**: after a backend contract change, run `npm run gen:api-types` (backend up on `localhost:8000`) then `npm run typecheck` — a renamed/removed field then fails compile at the consumer. Exceptions (`/billing/status`, `/video-url/` have no `response_model`) stay hand-written. Never hand-edit `lib/api/schema.d.ts`.
